...
public class Seeder(RoleManager<IdentityRole> roleManager, UserManager<UserEntity> userManager, AppDbContext dbContext)
{
    ...
    private async Task SeedProducts()
    {
        ...
        var colorHexes = new[]
            { "#000000", "#FFFFFF", "#0000FF", "#FF0000", "#008000", "#FFFF00", "#808080", "#A52A2A" };
        ...
        foreach (var product in products)
        {
            ...
            Console.WriteLine(
                $"Added {selectedColors.Count} colors (total {productDetailsCount} product details) for product: {product.Name}");
        }
    ...
    private async Task SeedUsers()
    {
        ...
        foreach (var t in userEmails)
        {
            ...
            {
                Console.WriteLine(
                    $"Failed to create user({t}): {string.Join(", ", result.Errors.Select(e => e.Description))}");
            }
        }
        ...
        foreach (var user in users)
        {
            ...
            cartItems.AddRange(selectedProducts.Select(productDetail => new CartItemEntity
            {
                UserId = user.Id,
